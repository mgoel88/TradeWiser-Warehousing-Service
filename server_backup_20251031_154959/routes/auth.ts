...
authRouter.post('/login', async (req: Request, res: Response) => {
  try {
    ...
    req.session.userId = user.id;
    console.log('🔑 [LOGIN] Session before save:', JSON.stringify(req.session));
    req.session.save((err) => {
      if (err) {
...
authRouter.post('/register', async (req: Request, res: Response) => {
  try {
    ...
    req.session.save((err) => {
      if (err) {
        console.error('Session save error (registration):', err);
        return res.status(500).json({ success: false, message: 'Failed to save session' });
      }
...
authRouter.post('/social-login', async (req: Request, res: Response) => {
  try {
    ...
    req.session.save((err) => {
      if (err) {
        console.error('Session save error (username/password):', err);
        return res.status(500).json({ success: false, message: 'Failed to save session' });
      }
      console.log('✅ Session saved successfully. Session after save:', req.session);
